...
function coachingLimitationIssues(exercise,spec) {
  ...
  limitations.forEach((tag) => {
    ...
    issues.push({
      ...
      limitation:tag,
      label:(INJURY_LABELS[tag] || tag) + " conflict" + (rule.blockAll ? " · exercise is paused until cleared" : detail ? " · " + detail : impactConflict ? " · impact level" : " · high-intensity finisher"),
      hard:true,
    });
  });
  return issues;
...
function exerciseConstraintIssues(exercise,spec,age) {
  ...
  limitations.forEach((tag) => { if (avoid.includes(tag)) issues.push({ code:"limitation",limitation:tag,label:(INJURY_LABELS[tag] || tag) + " conflict · exercise-specific exclusion",hard:true }); });
  issues.push(...coachingLimitationIssues(exercise,spec || {}));
  if (Number(exercise.exp || 1) > Number(spec && spec.experience || 1)) issues.push({ code:"experience",label:"Above selected experience",hard:true });
  if (Number(exercise.impact || 0) > prof.impactCap || (exercise.pattern === "plyo" && !prof.plyoOK)) issues.push({ code:"impact",label:"Age / impact caution",hard:true });
  const preference = exercisePreferenceFor(spec || {},exercise); if (["discomfort","unavailable"].includes(preference)) issues.push({ code:"preference",label:preference === "discomfort" ? "Marked as causing discomfort" : "Marked unavailable",hard:true });
