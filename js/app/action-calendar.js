...
const CALENDAR_TYPE_LABELS = { appointment:"Appointment", workout:"Workout", followup:"Follow-up", admin:"Team task" };
...
function calendarTier(profile) { return String(profile && (profile.serviceTier || profile.membershipTier || profile.tier) || "General"); }
...
function ensureCalendarEventDialog() {
  ...
  wrapper.innerHTML = '<div id="calendarEventModal" class="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="calendarEventTitle"><div class="review-dialog calendar-event-dialog"><div class="calendar-modal-head"><div><span id="calendarEventSource" class="client-section-label">Calendar item</span><h2 id="calendarEventTitle">Schedule coaching work</h2></div><button class="icon-close" type="button" onclick="closeCalendarEventEditor()" aria-label="Close">×</button></div><input id="calendarEventId" type="hidden"><input id="calendarEventSourceId" type="hidden"><div class="calendar-event-form"><div class="compact-field"><label for="calendarEventType">Type</label><select id="calendarEventType"><option value="appointment">Appointment</option><option value="followup">Follow-up</option><option value="admin">Team task</option></select></div><div class="compact-field"><label for="calendarEventStatus">Status</label><select id="calendarEventStatus"><option value="scheduled">Scheduled</option><option value="rescheduled">Rescheduled</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option><option value="missed">Missed</option></select></div><div class="compact-field wide"><label for="calendarEventTitleInput">Title</label><input id="calendarEventTitleInput" maxlength="100" placeholder="Training appointment or follow-up"></div><div class="compact-field"><label for="calendarEventClient">Client</label><select id="calendarEventClient" onchange="calendarEventClientChanged()"></select></div><div class="compact-field"><label for="calendarEventTrainer">Trainer</label><select id="calendarEventTrainer"></select></div><div class="compact-field"><label for="calendarEventDate">Date</label><input id="calendarEventDate" type="date"></div><div class="compact-field"><label for="calendarEventStart">Start</label><input id="calendarEventStart" type="time"></div><div class="compact-field"><label for="calendarEventEnd">End</label><input id="calendarEventEnd" type="time"></div><div class="compact-field wide"><label for="calendarEventLocation">Location</label><input id="calendarEventLocation" maxlength="100" placeholder="Fit4Life, office, phone, or video"></div><div class="compact-field wide"><label for="calendarEventNotes">Internal details</label><textarea id="calendarEventNotes" rows="3" placeholder="Preparation or coaching context"></textarea></div><div class="compact-field wide"><label for="calendarEventReason">Reason for a changed date, time, or status</label><textarea id="calendarEventReason" rows="2" placeholder="Required when changing an existing schedule"></textarea></div><label class="calendar-notify-check"><input id="calendarEventNotify" type="checkbox" checked> Record a client/team schedule notice</label></div><div id="calendarConflictNote" class="calendar-policy-note" hidden></div><div class="tool-actions"><button id="calendarEventSaveBtn" class="small-btn primary" type="button" onclick="saveCalendarEvent()">Save calendar item</button><button class="small-btn" type="button" onclick="closeCalendarEventEditor()">Cancel</button></div><details class="formal-review-box" id="calendarAuditBox"><summary>Change history</summary><div id="calendarEventAudit" class="calendar-audit-list"></div></details></div></div>';
...
function openCalendarEventEditor(eventId,dateKey,type) {
  ...
  byId("calendarEventSource").textContent = sourceAssignment ? "Assigned workout · date only" : event ? "Calendar item" : "New calendar item";
  ...
  byId("calendarEventType").disabled = sourceAssignment; byId("calendarEventStatus").value = event && event.status || "scheduled"; byId("calendarEventStatus").disabled = sourceAssignment;
  byId("calendarEventTitleInput").value = event && event.title || (type === "followup" ? "Client follow-up" : "Training appointment"); byId("calendarEventTitleInput").disabled = sourceAssignment;
  byId("calendarEventClient").value = event && event.profileId || ""; byId("calendarEventClient").disabled = sourceAssignment;
  byId("calendarEventTrainer").value = event && event.trainerId || identity.id || ""; byId("calendarEventTrainer").disabled = sourceAssignment;
  ...
  byId("calendarEventStart").disabled = sourceAssignment; byId("calendarEventEnd").disabled = sourceAssignment;
  byId("calendarEventLocation").value = event && event.location || ""; byId("calendarEventLocation").disabled = sourceAssignment;
  byId("calendarEventNotes").value = event && event.notes || ""; byId("calendarEventNotes").disabled = sourceAssignment; byId("calendarEventReason").value = "";
  ...
  byId("calendarAuditBox").hidden = !event; modal.classList.add("open"); setTimeout(() => byId(sourceAssignment ? "calendarEventDate" : "calendarEventTitleInput").focus(),20);
}
function closeCalendarEventEditor() { const modal = byId("calendarEventModal"); if (modal) modal.classList.remove("open"); }
...
function saveCalendarEvent() {
  ...
  const eventId = byId("calendarEventId").value, sourceId = byId("calendarEventSourceId").value, existing = eventId ? calendarEventById(eventId) : null, reason = byId("calendarEventReason").value.trim(), date = byId("calendarEventDate").value;
  ...
  if (sourceId) {
    ...
    const before = {...assignment}; if (assignment.scheduledDate === date) { showToast("Choose a different workout date before saving"); return false; }
    if (!reason) { showToast("Add the reason for changing this workout date"); return false; }
    assignment.scheduledDate = date; assignment.updatedAt = new Date().toISOString(); assignment.updatedBy = calendarIdentity().name;
    ...
    closeCalendarEventEditor(); coachCalendarState.anchor = date; renderCoachCalendarModule(); renderTrainerAttention(); showToast("Workout date updated and recorded"); return true;
  }
...
function calendarEventChip(event) {
  return '<button class="calendar-event-chip ' + escapeHtml(event.type) + ' ' + escapeHtml(event.status) + '" onclick="openCalendarEventEditor(\'' + escapeHtml(event.id) + '\')"><span>' + escapeHtml(event.startTime ? calendarFormatTime(event.startTime) : event.type === "workout" ? "Workout" : "All day") + '</span><b>' + escapeHtml(event.client ? event.client + " · " + event.title : event.title) + '</b><em>' + escapeHtml(event.trainerName || "Shared team") + '</em></button>';
}
...
function renderCoachCalendarModule() {
  ...
  const profiles = loadProfiles(), trainers = calendarTrainerOptions(), tiers = [...new Set(profiles.map(calendarTier))].sort(), events = allCoachCalendarEvents().filter(calendarEventMatches);
