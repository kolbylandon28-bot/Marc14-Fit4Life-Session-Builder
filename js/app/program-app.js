...
function trainerAttentionSnapshot() {
  ...
  const intakeReviews = profiles.filter((profile) => {
...
function renderTrainerAttention() {
  ...
  panel.innerHTML =
    ...
    '<button class="attention-card ' + (attention.reviews ? 'urgent' : '') + '" onclick="openCoachDestination(\'calendar\')"><span>Reviews waiting</span><b>' + attention.reviews + '</b><p>' + attention.intakeReviews.length + ' onboarding · ' + attention.assignmentReviews.length + ' workout · ' + attention.checkinReviews.length + ' check-in · ' + attention.receiptRequests.length + ' receipt' + (attention.recoveryOverdue.length ? ' · ' + attention.recoveryOverdue.length + ' recovery overdue' : '') + '</p></button>' +
