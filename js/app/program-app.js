...
function renderClientHome(profile) {
  ...
  const assignments = assignmentsForClient(profile.id), assignment = assignments.find((item) => ["assigned","in_progress"].includes(assignmentStatus(item))) || null, latestFinished = assignments.find((item) => ["completed","reviewed"].includes(assignmentStatus(item))) || null, session = clientAssignedSession(assignment,profile);
  ...
  const recoveryStatus = recoveryFollowUpStatus(profile), recoveryCard = recoveryStatus.active && recoveryStatus.due
    ...
    : '';
  const checkInCard = '<section class="client-card"><div class="client-section-label">Weekly check-in</div><h3>' + (lastCheckIn ? 'Next review · ' + nextCheck.toLocaleDateString() : 'Ready when you are') + '</h3><p>' + (lastCheckIn ? 'Your last update was sent ' + new Date(lastCheckIn.createdAt || lastCheckIn.date).toLocaleDateString() + '.' : 'Share recovery, wins, barriers, and schedule changes with your coach.') + '</p><div class="tool-actions"><button class="small-btn" onclick="openClientCheckInForActive()">Open check-in</button></div></section>';
  ...
  const review = latestFinished && latestFinished.clientReview, completionSummary = review ? '<section class="client-card wide"><div class="client-section-label">Latest workout summary</div><div class="simple-stat-grid"><div class="simple-stat"><b>' + (review.actualDuration || review.duration || '—') + '</b><span>Minutes</span></div><div class="simple-stat"><b>' + (review.loggedSets || 0) + '</b><span>Working sets</span></div><div class="simple-stat"><b>' + (review.difficulty || '—') + (review.difficulty ? '/10' : '') + '</b><span>Difficulty</span></div><div class="simple-stat"><b>' + ((review.personalRecords || []).length) + '</b><span>Personal records</span></div></div><p style="margin-top:12px">' + (review.pain === 'none' ? 'No pain reported.' : 'Pain or discomfort reported: ' + escapeHtml(review.injuryArea || review.pain || 'not specified') + '.') + ((review.personalRecords || []).length ? ' Personal bests: ' + escapeHtml(review.personalRecords.join(', ')) + '.' : '') + (review.notes ? ' ' + escapeHtml(review.notes) : '') + '</p><div class="tool-actions"><button class="small-btn" onclick="openClientTab(\'progress\')">View workout history</button></div></section>' : '';
  ...
  const request = clientWorkoutRequestStatus(profile), headline = session ? session.goalLabel || 'Coach-planned workout' : request.pending ? 'Workout requested' : 'Ready for your next workout';
  byId("clientHomeGreeting").textContent = "Hi, " + String(profile.name).split(/\s+/)[0]; byId("clientHomeSubhead").textContent = assignment ? clientWorkoutStatusLabel(assignment) : request.pending ? 'Your coach received your request' : 'No active workout assigned'; byId("clientHomeAvatar").textContent = clientInitials(profile);
  out.innerHTML = '<section class="client-hero"><div class="hero-kicker">' + (assignment ? 'Current workout' : 'Next coaching action') + '</div><h2>' + escapeHtml(headline) + '</h2><div class="hero-meta"><span>' + escapeHtml(session ? (session.spec.minutes || 60) + ' min' : 'One workout at a time') + '</span><span>' + escapeHtml(session ? clientSessionEquipment(session).slice(0,3).join(' · ') || 'Bodyweight' : profile.assignedTrainerName || 'Coaching team') + '</span><span>' + escapeHtml(assignment ? clientWorkoutStatusLabel(assignment) : request.pending ? 'Request sent' : 'Waiting for assignment') + '</span></div><p class="hero-purpose">' + escapeHtml(session ? clientPurpose(session) : request.pending ? 'Your trainer has been notified. This page will update when the next workout is assigned.' : 'Let your trainer know you are ready. They will build and assign one clear workout for you.') + '</p>' + (assignment ? '<button class="start-workout-btn" onclick="startActiveWorkout(\'' + escapeHtml(profile.id) + '\')">' + (assignmentStatus(assignment) === 'in_progress' ? 'Continue workout' : 'Start workout') + '</button>' : '<button class="start-workout-btn" ' + (request.pending ? 'disabled' : '') + ' onclick="requestNextWorkout()">' + (request.pending ? 'Workout requested' : 'Request next workout') + '</button>') + '</section>'
    + '<div class="client-grid">' + (latestFinished && latestFinished.coachNote ? '<section class="client-card"><div class="client-section-label">Coach feedback</div><h3>Update from your coach</h3><p>' + escapeHtml(latestFinished.coachNote) + '</p><div class="tool-actions"><button class="small-btn" onclick="openClientTab(\'coach\')">Open coach</button></div></section>' : '')
    + receiptCard + recoveryCard + completionSummary + checkInCard + competition + '</div>';
}
function clientWorkoutRequestStatus(profile) {
  const messages = loadClientMessages(profile.id), latestRequest = messages.find((item) => item.messageType === "workout_request");
  if (!latestRequest) return {pending:false,request:null};
  const answered = messages.some((item) => messageSenderRole(item) !== "client" && new Date(item.createdAt || 0) > new Date(latestRequest.createdAt || 0));
  const assignedAfter = assignmentsForClient(profile.id).some((item) => ["assigned","in_progress"].includes(assignmentStatus(item)) && new Date(item.assignedAt || 0) > new Date(latestRequest.createdAt || 0));
  return {pending:!answered && !assignedAfter,request:latestRequest};
}
function requestNextWorkout() {
  const profile = activeClientProfile(); if (!profile) return null;
  const status = clientWorkoutRequestStatus(profile); if (status.pending) { showToast("Your trainer already has your workout request"); return status.request; }
  const identity = currentAccountIdentity(), messages = loadLocalArray(CLIENT_MESSAGES_KEY), request = {id:"workout-request-" + Date.now(),profileId:profile.id,client:profile.name,from:"client",senderRole:"client",senderUserId:identity.id || "",senderName:profile.name,recipientRole:"trainer",recipientUserId:profile.assignedTrainerId || "",recipientName:profile.assignedTrainerName || "Coaching team",messageType:"workout_request",body:"I’m ready for my next workout.",createdAt:new Date().toISOString()};
  messages.unshift(request); if (!writeLocalArray(CLIENT_MESSAGES_KEY,messages,1000)) return null;
  renderClientHome(profile); renderTrainerAttention(); showToast("Workout request sent to " + (profile.assignedTrainerName || "your coaching team")); return request;
}
...
function renderClientProgram(profile) {
  const out = byId("clientProgramContent"); if (!out) return;
  const assignments = assignmentsForClient(profile.id), assignment = assignments.find((item) => ["assigned","in_progress"].includes(assignmentStatus(item))) || null, session = clientAssignedSession(assignment,profile), request = clientWorkoutRequestStatus(profile);
  const preview = session ? renderClientWorkoutPreview(profile,session,assignment) : '<section class="client-card wide workout-empty"><div class="client-section-label">Current workout</div><h3>' + (request.pending ? 'Your request is with your coach' : 'No active workout assigned') + '</h3><p>' + (request.pending ? 'You will see the new workout here as soon as it is assigned.' : 'Request one workout when you are ready to train again.') + '</p><div class="tool-actions"><button class="small-btn primary" ' + (request.pending ? 'disabled' : '') + ' onclick="requestNextWorkout()">' + (request.pending ? 'Request sent' : 'Request next workout') + '</button></div></section>';
  const history = assignments.filter((item) => ["completed","reviewed"].includes(assignmentStatus(item))).slice(0,8);
  out.innerHTML = preview + '<section class="client-card wide client-workout-history"><div class="client-section-label">Workout history</div><h3>Your completed sessions</h3><div class="advanced-list">' + (history.map((item) => { const review = item.clientReview || {}, date = item.completedAt || item.assignedAt; return '<div class="advanced-list-item"><div><b>' + new Date(date).toLocaleDateString() + ' · ' + escapeHtml(assignmentStatusLabel(item)) + '</b><span>' + (review.loggedSets || assignmentProgressStats(item).logged) + ' sets · difficulty ' + (review.difficulty || '—') + (review.difficulty ? '/10' : '') + ' · ' + (review.pain === 'none' ? 'no pain reported' : review.pain ? 'pain: ' + escapeHtml(review.injuryArea || review.pain) : 'pain not recorded') + '</span></div></div>'; }).join('') || '<div class="empty-state">No completed workouts yet.</div>') + '</div><div class="tool-actions"><button class="small-btn" onclick="openClientTab(\'progress\')">Open full progress</button></div></section>';
}
...
function renderClientWorkoutPreview(profile,session,assignment) {
  ...
  return '<section class="workout-preview"><article class="client-card"><div class="client-section-label">Current workout</div><h3>' + escapeHtml(session.goalLabel || 'Coach-planned workout') + '</h3><div class="hero-meta"><span>' + (session.spec.minutes || 60) + ' min</span><span>' + escapeHtml(equipment.join(' · ') || 'Bodyweight') + '</span><span>' + items.length + ' exercises</span></div><p>' + escapeHtml(clientPurpose(session)) + '</p><div class="preview-list">' + items.map((item,index) => '<div class="preview-exercise"><i>' + (index + 1) + '</i><b>' + escapeHtml(item.exercise.name) + '<small style="display:block;color:var(--text-faint);margin-top:3px">' + escapeHtml(item.block.title) + '</small></b><span>' + escapeHtml((item.exercise.rx || item.block.rx).sets + ' × ' + (item.exercise.rx || item.block.rx).reps) + '</span></div>').join('') + '</div></article><aside class="client-card"><div class="client-section-label">Workout options</div>' + (!assignment ? '<div class="portal-note">Your trainer has not assigned this workout yet.</div>' : '') + '<div class="secondary-actions"><button class="small-btn primary" ' + (ready ? '' : 'disabled') + ' onclick="startActiveWorkout(\'' + escapeHtml(profile.id) + '\',false,\'' + escapeHtml(assignmentId) + '\')">' + (assignmentStatus(assignment) === 'in_progress' ? 'Continue workout' : assignment && ["completed","reviewed"].includes(assignmentStatus(assignment)) ? 'Workout complete' : 'Start workout') + '</button><button class="small-btn" ' + (ready ? '' : 'disabled') + ' onclick="startActiveWorkout(\'' + escapeHtml(profile.id) + '\',true,\'' + escapeHtml(assignmentId) + '\')">Use shortened workout</button><button class="small-btn" onclick="showToast(\'Start the workout, then use Replace exercise beside the movement that needs different equipment\')">Change equipment</button><button class="small-btn" onclick="openClientPainReport()">Report a limitation</button></div><p style="margin-top:12px">Exercise changes remain inside trainer-approved safety and equipment rules.</p></aside></section>';
}
...
function trainerAttentionSnapshot() {
  ...
  const intakeReviews = FIT4LIFE_RELEASE.onboardingVisible ? profiles.filter((profile) => {
    const status = intakeCompletion(profile);
    return Boolean(profile.intake && status.clientComplete && !status.trainerReviewComplete);
  }) : [];
  ...
  const clientAccessRequests = (window.fit4lifeCloudRegistrationRequests || []).filter((request) => request.requested_role !== "trainer" && request.status === "pending");
  const profileOnlyRequests = loadProfileRequests();
  const items = [];
  ...
  profiles.forEach((profile) => {
    ...
    if (latestMessage && messageSenderRole(latestMessage) === "client") {
      const workoutRequest = latestMessage.messageType === "workout_request";
      if (!workoutRequest || clientWorkoutRequestStatus(profile).pending) {
        push({id:"message:" + latestMessage.id,profileId:profile.id,client:profile.name,trainer:profile.assignedTrainerName || "Coaching team",kind:workoutRequest ? "workout_request" : "message",urgency:workoutRequest ? "high" : "urgent",rank:workoutRequest ? 1 : 0,createdAt:latestMessage.createdAt,label:workoutRequest ? "Next workout requested" : "Unanswered message",detail:latestMessage.body || (workoutRequest ? "Client is ready for another workout." : "Client is waiting for a response.")});
      }
    }
    ...
    if (FIT4LIFE_RELEASE.onboardingVisible) {
    ...
    if (FIT4LIFE_RELEASE.programsVisible) {
  ...
  clientAccessRequests.forEach((request) => push({id:"client-access-request:" + request.id,profileId:"",client:request.full_name || request.email || "Client account",trainer:"Client access",kind:"client_access_request",urgency:"normal",rank:3,createdAt:request.created_at,label:"Client account request",detail:"Review the verified client request and connect it to the correct protected profile."}));
  profileOnlyRequests.forEach((request) => push({id:"client-profile-request:" + request.id,profileId:"",client:request.name || request.username || "Client profile",trainer:"Client access",kind:"client_profile_request",urgency:"normal",rank:3,createdAt:request.createdAt,label:"Client profile requested",detail:"Check for duplicates, then approve and create the protected client profile."}));
  const attentionState = loadAttentionState(), visibleItems = items.filter((item) => attentionItemIsVisible(item,attentionState)).map((item) => ({...item,opened:attentionState[item.id] && attentionState[item.id].status === "opened"})).sort((a,b) => Number(a.rank || 9) - Number(b.rank || 9) || new Date(a.createdAt || 0) - new Date(b.createdAt || 0));
  return {unanswered,assignmentReviews,checkinReviews,intakeReviews,recoveryOverdue,formalReviews,receiptRequests,recognition,trainerRequests,clientAccessRequests,profileOnlyRequests,automationAlerts,reviews:intakeReviews.length + assignmentReviews.length + checkinReviews.length + receiptRequests.length,items:visibleItems};
}
function openCoachAttentionItem(profileId,kind,itemId) {
  markCoachAttentionOpened(itemId);
  if (kind === "trainer_request") { openCoachDestination("access"); return; }
  if (["client_access_request","client_profile_request"].includes(kind)) { openCoachDestination("clients"); setTimeout(() => { const queue = byId("profileRequestQueue"); if (queue) queue.scrollIntoView({behavior:"smooth",block:"start"}); },30); return; }
  ...
  if (kind === "workout_request") { selectedTrainerClient = profile.name; openSelectedClientSession(); return; }
  if (kind === "automation") { openAdvancedForClient(profile.id,"review"); return; }
  selectedTrainerClient = profile.name;
  selectedInBodyScanId = "";
  trainerSummaryState = newTrainerSummaryState();
  trainerSummaryState.tab = kind === "message" || kind === "recognition" || kind === "inactive" || kind === "recovery_due" ? "communication"
  ...
  else if (["message","recognition","inactive","recovery_due"].includes(kind)) setTimeout(() => replyToClientMessage(profile.id),30);
}
function openFirstAccessRequest() {
  const item = trainerAttentionSnapshot().items.find((entry) => ["trainer_request","client_access_request","client_profile_request"].includes(entry.kind));
  if (item) { openCoachAttentionItem(item.profileId,item.kind,item.id); return; }
  openCoachDestination("clients");
}
function markCoachAttentionOpened(id) {
  if (!id) return;
  const state = loadAttentionState(), current = state[id];
  if (!current || current.status === "opened") {
    state[id] = {status:"opened",openedAt:current && current.openedAt || new Date().toISOString(),updatedAt:new Date().toISOString(),updatedBy:currentAccountIdentity().displayName};
...
function renderTrainerAttention() {
  ...
  document.querySelectorAll("[data-attention-badge]").forEach((badge) => {
    const key = badge.dataset.attentionBadge, count = key === "messages" ? attention.unanswered.length : key === "reviews" ? attention.reviews : attention.trainerRequests.length + attention.clientAccessRequests.length + attention.profileOnlyRequests.length;
  ...
  panel.innerHTML =
    ...
    '<button class="attention-card" onclick="openFirstAccessRequest()"><span>Access requests</span><b>' + (attention.trainerRequests.length + attention.clientAccessRequests.length + attention.profileOnlyRequests.length) + '</b><p>' + attention.trainerRequests.length + ' trainer · ' + (attention.clientAccessRequests.length + attention.profileOnlyRequests.length) + ' client request' + (attention.clientAccessRequests.length + attention.profileOnlyRequests.length === 1 ? '' : 's') + '</p></button>' +
    ...
    (attention.items.slice(0,12).map((item) => '<article class="attention-item ' + escapeHtml(item.urgency) + (item.opened ? ' opened' : '') + '"><i class="attention-priority" aria-hidden="true"></i><div class="attention-person"><b>' + escapeHtml(item.client || "Workspace") + '</b><span>' + escapeHtml(item.trainer || "Coaching team") + ' · waiting ' + escapeHtml(attentionWaitingLabel(item.createdAt)) + (item.opened ? ' · opened' : '') + '</span></div><div class="attention-reason"><b>' + escapeHtml(item.label) + '</b><span>' + escapeHtml(item.detail) + '</span></div><div class="attention-item-actions"><button class="mini-btn primary" onclick="openCoachAttentionItem(\'' + escapeHtml(item.profileId || "") + '\',\'' + escapeHtml(item.kind) + '\',\'' + escapeHtml(item.id || "") + '\')">' + (item.opened ? 'Open again' : 'Open') + '</button><button class="mini-btn" onclick="updateCoachAttentionItem(\'' + escapeHtml(item.id) + '\',\'snoozed\')">Tomorrow</button><button class="mini-btn" onclick="updateCoachAttentionItem(\'' + escapeHtml(item.id) + '\',\'done\')">Handled</button></div></article>').join("") || '<div class="attention-empty"><b>No client is waiting on the coaching team.</b><br>New workout requests, pain reports, messages, and completed sessions will appear here.</div>') +
    '</div></section>';
  return attention;
}
...
function renderClientCoach(profile) {
  ...
  out.innerHTML = '<div class="client-grid"><section class="client-card wide"><div class="client-section-label">Direct messages · ' + escapeHtml(profile.assignedTrainerName || 'Coaching team') + '</div>' + messageThreadHtml(messages,profile,'client') + '<div class="message-compose"><div class="compact-field"><label for="clientMessageInput">Message ' + escapeHtml(profile.assignedTrainerName || 'your coaching team') + '</label><input id="clientMessageInput" placeholder="Ask one clear question" onkeydown="if(event.key===\'Enter\')sendClientMessage()"></div><button class="small-btn primary" onclick="sendClientMessage()">Send message</button></div></section>'
    ...
    + '<section class="client-card"><div class="client-section-label">Workout status</div><h3>' + escapeHtml(clientWorkoutStatusLabel(assignment)) + '</h3><p>Your active workout and trainer review status stay separate from direct messages.</p></section>'
...
function renderActiveWorkout() {
  ...
  const readinessBanner = readinessToday ? '<section class="active-readiness-banner"><div class="active-readiness-score">' + escapeHtml(readinessToday.score) + '</div><div class="active-readiness-copy"><b>' + escapeHtml(readinessToday.title || 'Today’s readiness adjustment') + '</b><p>' + escapeHtml((readinessToday.changes || []).join(' · ') || 'The assigned workout is being used as written.') + '</p><small>Today only · the trainer-approved workout stays on record</small></div><span>Pre-workout check</span></section>' : '';
...
function approveCurrentWorkoutDraft() {
  ...
  if (FIT4LIFE_RELEASE.onboardingVisible && intakeStatus && intakeStatus.approvalBlocked) { showToast("Coach approval blocked — finish " + intakeProfile.name + "’s onboarding and document the trainer baseline decision first"); return false; }
...
function assignCurrentWorkout() {
  ...
  workoutPlans(state.session).forEach((plan) => {
    ...
    const unfinished = assignments.find((item) => item.profileId === profile.id && ["in_progress","completed"].includes(assignmentStatus(item)));
    if (unfinished) {
      blocked.push(profile.name + (assignmentStatus(unfinished) === "completed" ? ": review the completed workout before assigning the next one" : ": the current workout is already in progress"));
      return;
    }
    const assignedAt = new Date().toISOString(), session = { type:"solo", data:JSON.parse(JSON.stringify(plan.session)), edits:JSON.parse(JSON.stringify(state.session.edits || {})) }, assignment = { id:"assignment-" + Date.now() + "-" + Math.random().toString(16).slice(2), profileId:profile.id, client:profile.name, assignedAt, status:"assigned", session };
    assignments.forEach((item) => {
      if (item.profileId === profile.id && assignmentStatus(item) === "assigned") {
        item.status = "superseded";
        item.supersededAt = assignedAt;
        item.supersededBy = assignment.id;
      }
    });
...
function openSingleWorkoutBuilder() {
  if (!trainerIsUnlocked()) { requestTrainerAccess("workouts"); return; }
  portalRole = "trainer"; openCoachDestination.current = "workouts"; renderForms(); syncBuilderRoleCopy(); show("builder");
}
function openCoachDestination(destination) {
  if (!trainerIsUnlocked()) { requestTrainerAccess(destination === 'clients' || destination === 'reports' ? 'trainer' : destination === 'workouts' ? 'workouts' : 'trainer-menu'); return; }
  ...
  if (destination === 'workouts' || destination === 'programming') { openSingleWorkoutBuilder(); return; }
  if (destination === 'assessments') { openReadiness(); return; }
  renderCoachModule(destination); show('coach-module');
}
let coachCalendarOffset = 0;
let coachMessageSearchQuery = '';
function coachCalendarWeek() {
  const today = new Date(), monday = new Date(today); monday.setHours(12,0,0,0); monday.setDate(today.getDate() - (today.getDay() === 0 ? 6 : today.getDay() - 1) + coachCalendarOffset * 7);
  return ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"].map((label,index) => { const date = new Date(monday); date.setDate(monday.getDate() + index); return {weekday:index + 1,label,date,dateKey:date.toISOString().slice(0,10)}; });
}
function changeCoachCalendarWeek(change) { coachCalendarOffset += Number(change) || 0; renderCoachModule("calendar"); }
function resetCoachCalendarWeek() { coachCalendarOffset = 0; renderCoachModule("calendar"); }
function filterCoachMessageThreads(value) {
  coachMessageSearchQuery = String(value == null ? '' : value).trim().toLowerCase();
  let visible = 0;
  document.querySelectorAll('[data-message-thread]').forEach((thread) => {
    const match = !coachMessageSearchQuery || String(thread.dataset.messageSearch || '').includes(coachMessageSearchQuery);
    thread.hidden = !match; if (match) visible += 1;
  });
  const count = byId('coachMessageSearchCount'), empty = byId('coachMessageSearchEmpty');
  if (count) count.textContent = visible + ' conversation' + (visible === 1 ? '' : 's') + ' shown';
  if (empty) empty.hidden = visible !== 0;
  return visible;
}
function openCalendarClient(profileId,tab) {
  const profile = loadProfiles().find((item) => item.id === profileId); if (!profile) return;
  selectedTrainerClient = profile.name; selectedInBodyScanId = ""; trainerSummaryState = newTrainerSummaryState(); trainerSummaryState.tab = tab === "workout" ? "workouts" : "overview"; show("trainer"); renderTrainerHub(profile.name);
}
function scheduleAssignmentDate(assignmentId,dateValue) {
  if (!requireTrainerMutation("schedule an assigned workout")) return false;
  const assignments = loadAssignedWorkouts(), index = assignments.findIndex((item) => item.id === assignmentId); if (index < 0) return false;
  assignments[index].scheduledDate = dateValue || "";
  assignments[index].scheduledWeekday = dateValue ? ((new Date(dateValue + "T12:00:00").getDay() || 7)) : null;
  assignments[index].updatedAt = new Date().toISOString();
  if (!writeAssignedWorkouts(assignments)) return false;
  renderCoachModule("calendar"); showToast(dateValue ? "Workout date updated" : "Workout moved to the unscheduled list"); return true;
}
function attendanceCalendarHtml() {
  const profiles = loadProfiles(), assignments = loadAssignedWorkouts().filter((item) => !["superseded","cancelled"].includes(assignmentStatus(item))), todayKey = new Date().toISOString().slice(0,10), week = coachCalendarWeek();
  const first = week[0].date, last = week[week.length - 1].date, rangeLabel = first.toLocaleDateString([],{month:"short",day:"numeric"}) + " – " + last.toLocaleDateString([],{month:"short",day:"numeric",year:"numeric"});
  const days = coachCalendarWeek().map((day) => {
    const scheduled = assignments.filter((item) => item.scheduledDate === day.dateKey).sort((a,b) => String(a.client).localeCompare(String(b.client)));
    const rows = scheduled.map((assignment) => {
      const profile = profiles.find((item) => item.id === assignment.profileId), status = assignmentStatus(assignment);
      return '<article class="calendar-workout-event ' + escapeHtml(status) + '"><div><b>' + escapeHtml(assignment.client) + '</b><span>' + escapeHtml(assignmentStatusLabel(assignment)) + '</span></div><div class="calendar-event-actions"><button class="tiny-btn" type="button" onclick="openCalendarClient(\'' + escapeHtml(profile && profile.id || assignment.profileId) + '\',\'profile\')">Client</button><button class="tiny-btn primary" type="button" onclick="openCalendarClient(\'' + escapeHtml(profile && profile.id || assignment.profileId) + '\',\'workout\')">Workout</button></div></article>';
    }).join("");
    return '<section class="attendance-day' + (day.dateKey === todayKey ? ' today' : '') + '"><div class="attendance-day-head"><b>' + day.label + '</b><span>' + day.date.toLocaleDateString([],{month:"short",day:"numeric"}) + '</span></div>' + (rows || '<div class="attendance-empty">No workouts scheduled.</div>') + '</section>';
  }).join("");
  const unscheduled = assignments.filter((item) => ["assigned","in_progress"].includes(assignmentStatus(item)) && !item.scheduledDate);
  const unscheduledHtml = '<section class="coach-module-card calendar-unscheduled"><div class="attendance-head"><div><h3>Needs a workout date</h3><p>Active assignments without a date stay here instead of appearing on every available training day.</p></div><span class="attention-command-count">' + unscheduled.length + '</span></div><div class="advanced-list">' + (unscheduled.map((item) => '<div class="calendar-unscheduled-row"><div><b>' + escapeHtml(item.client) + '</b><span>' + escapeHtml(assignmentStatusLabel(item)) + '</span></div><label>Workout date<input type="date" onchange="scheduleAssignmentDate(\'' + escapeHtml(item.id) + '\',this.value)"></label><button class="tiny-btn" onclick="openCalendarClient(\'' + escapeHtml(item.profileId) + '\',\'workout\')">Open</button></div>').join('') || '<div class="empty-state">Every active workout has a date.</div>') + '</div></section>';
  return '<section class="attendance-calendar"><div class="attendance-head calendar-toolbar"><div><h3>Workout calendar</h3><p>' + escapeHtml(rangeLabel) + ' · scheduled workouts use their assigned date and status.</p></div><div class="calendar-toolbar-actions"><button class="small-btn" onclick="changeCoachCalendarWeek(-1)">← Previous</button><button class="small-btn" onclick="resetCoachCalendarWeek()">Today</button><button class="small-btn" onclick="changeCoachCalendarWeek(1)">Next →</button></div></div><div class="calendar-legend"><span class="assigned">Assigned</span><span class="in_progress">In progress</span><span class="completed">Needs review</span><span class="reviewed">Reviewed</span></div><div class="attendance-grid">' + days + '</div></section>' + unscheduledHtml;
}
function renderCoachModule(destination) {
  ...
  const details = {
    calendar:['Calendar','Schedule workouts and open every follow-up at the exact action it needs.'],messages:['Messages','Review direct questions separately from workout feedback and check-ins.'],library:['Exercise Library','Search the gym-approved movement bank and review substitution rules.'],access:['Trainer Access','Approve named trainer accounts and keep staff permissions separate from client registration.'],settings:['Settings','Manage workspace behavior, exports, account safety, and connected services.']
  ...
  if (destination === 'calendar') {
    const attention = trainerAttentionSnapshot(), followups = attention.items.filter((item) => !["recognition","inactive"].includes(item.kind));
    out.innerHTML = attendanceCalendarHtml() + '<section class="coach-module-card calendar-followups"><div class="attendance-head"><div><h3>Action queue</h3><p>Open each item at the exact client, workout, message, or approval screen that can resolve it.</p></div><span class="attention-command-count">' + followups.length + '</span></div><div class="advanced-list">' + (followups.map((item) => '<div class="calendar-followup-row ' + escapeHtml(item.urgency) + '"><div><b>' + escapeHtml(item.client) + ' · ' + escapeHtml(item.label) + '</b><span>' + escapeHtml(item.detail) + '</span></div><button class="tiny-btn primary" onclick="openCoachAttentionItem(\'' + escapeHtml(item.profileId || '') + '\',\'' + escapeHtml(item.kind) + '\',\'' + escapeHtml(item.id) + '\')">Open</button></div>').join('') || '<div class="empty-state">No coaching action is waiting.</div>') + '</div></section>';
    ...
    out.innerHTML = '<section class="coach-module-card" style="grid-column:1/-1"><h3>Client conversations</h3><p>Waiting conversations stay first. Search by client name, username, or email to open the right thread quickly.</p><div class="message-search-toolbar"><div class="compact-field"><label for="coachMessageSearch">Find a client conversation</label><input id="coachMessageSearch" type="search" autocomplete="off" value="' + escapeHtml(coachMessageSearchQuery) + '" placeholder="Name, username, or email" oninput="filterCoachMessageThreads(this.value)"></div><span id="coachMessageSearchCount">' + threads.length + ' conversation' + (threads.length === 1 ? '' : 's') + ' shown</span></div><div class="advanced-list" style="margin-top:14px">' + (threads.map((thread) => { const search = [thread.profile.name,profileUsername(thread.profile),thread.profile.email].filter(Boolean).join(' ').toLowerCase(); return '<details class="formal-review-box" data-message-thread data-message-search="' + escapeHtml(search) + '" ' + (thread.needsReply || threads.length === 1 ? 'open' : '') + '><summary>' + (thread.needsReply ? '<span class="tag tag-burn">Reply needed</span> ' : '<span class="tag">Up to date</span> ') + escapeHtml(thread.profile.name) + ' · @' + escapeHtml(profileUsername(thread.profile)) + '</summary>' + messageThreadHtml(thread.items,thread.profile,'trainer') + trainerMessageComposerHtml(thread.profile) + '</details>'; }).join('') || '<div class="empty-state">No direct client messages.</div>') + '<div class="empty-state" id="coachMessageSearchEmpty" hidden>No conversation matches that client search.</div></div></section>'; setTimeout(() => filterCoachMessageThreads(coachMessageSearchQuery),0);
...
function finishPendingTrainerDestination() {
  ...
  if (destination === "workouts") { openSingleWorkoutBuilder(); return; }
