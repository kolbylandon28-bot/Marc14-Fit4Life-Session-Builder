...
function saveSupersetEditor() {
  ...
  const target = context.block, source = data.sourceBlock, excluded = [data.first.name,data.second.name];
  if (source !== target) {
  ...
  showToast(data.first.name + " + " + data.second.name + " paired as A1/A2"); return true;
}
...
function savePrescriptionEditor() {
  ...
  const reps = byId("prescriptionReps").value.trim(), tempo = byId("prescriptionTempo").value.trim(), rest = byId("prescriptionRest").value.trim(), rpe = byId("prescriptionEffort").value.trim();
  if (!reps) { showToast("Enter reps or a duration"); return false; }
  const {session,block,ei,renderMode} = prescriptionEditContext, exercise = block.items[ei];
  exercise.rx = { ...(exercise.rx || block.rx || session.prescription || {}),sets:String(sets),reps,tempo:tempo || "controlled",rest:rest || "as needed",rpe:rpe || "coach set" };
  markSessionDraft(session,"Prescription edited");
  if (renderMode === "program" && currentProgram) {
    currentProgram.lifecycle = "draft";
    currentProgram.approval = {status:"draft",required:true,changedAt:new Date().toISOString(),changeReason:"Exercise prescription edited by coach"};
    byId("programApproveBtn").disabled = false; byId("programSaveBtn").disabled = true; byId("programSaveOnlyBtn").disabled = true;
  }
  closePrescriptionEditor(); if (renderMode === "program") renderProgram(); else renderOutput(); showToast(exercise.name + " prescription updated");
  return true;
}
...
function replaceExercise(block, ei, exercise) {
  ...
  if (block.groups) block.groups.forEach((group) => {
    const gi = group.items.indexOf(old);
    if (gi >= 0) group.items[gi] = replacement;
  });
  return true;
}
...
function applyExerciseSwap(exercise) {
  ...
  if (activeSwap.renderMode === "program") {
    const scope = byId("programSwapScope").value || "single", result = applyProgramExerciseReplacement(activeSwap,exercise,scope);
    if (!result.changed) { showToast(result.message || "No program exercises were changed"); return; }
    closeExerciseSwap(); renderProgram();
    showToast(exercise.name + " applied to " + result.changed + " workout" + (result.changed === 1 ? "" : "s") + (result.skipped ? " · " + result.skipped + " skipped to avoid duplicates" : ""));
    return;
  }
