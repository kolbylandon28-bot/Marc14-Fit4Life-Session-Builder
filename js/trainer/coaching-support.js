...
function renderClientCoachingModule() {
  ...
  return '<div class="advanced-grid">'
    ...
    + '<div id="aiCoachDraft" class="advanced-output">' + (currentAiCoachDraft ? escapeHtml(currentAiCoachDraft.text) : 'No decision brief yet. Building a brief does not mark the client’s check-in reviewed or change a workout. Only trainer approval saves the decision and opens the workout builder.') + '</div>'
...
function approveAiCoachDraft() {
  if (!requireTrainerMutation("approve an assisted workout draft") || !currentAiCoachDraft) return null;
  ...
  if (unreviewedIndex >= 0) { checkins[unreviewedIndex].reviewedAt = reviewedAt; writeCheckIns(checkins); }
  profiles[index] = { ...profiles[index], coachAdjustment:{ action:currentAiCoachDraft.action,note:currentAiCoachDraft.text,reviewedAt,source:"coach-decision-support" }, updatedAt:reviewedAt }; writeProfiles(profiles); selectedTrainerClient = profiles[index].name; showToast("Coach-approved brief saved and sent to the workout builder"); openSelectedClientSession(); return profiles[index];
}
...
function currentGymBrand() { return loadLocalObject(GYM_BRAND_KEY,{ name:"FIT 4 LIFE",sub:"BYU-Idaho Trainer Tools",primary:"#3E6BE0",accent:"#5AA6F0" }); }
...
function renderBrandModule() {
  const brand = currentGymBrand(), equipment = currentGymEquipment(), cloudTenant = window.fit4lifeCloudOrganizationSlug || "fit-4-life", portalUrl = location.origin + "/?gym=" + encodeURIComponent(cloudTenant);
...
function saveTeam() { if (!requireTrainerMutation("create teams")) return null; const name = byId("teamName").value.trim(); if (!name) { showToast("Add a team name"); return null; } const choices = document.querySelectorAll ? [...document.querySelectorAll(".team-member-choice:checked")].map((item) => item.value) : []; const teams = loadTeams(), team = { id:"team-" + Date.now(),name,coach:byId("teamCoach").value.trim(),focus:byId("teamFocus").value.trim(),profileIds:choices,createdAt:new Date().toISOString() }; teams.unshift(team); writeLocalArray(TEAMS_KEY,teams,200); renderAdvancedStudio(); showToast("Team created"); return team; }
function deleteTeam(id) { if (!requireTrainerMutation("delete teams")) return false; writeLocalArray(TEAMS_KEY,loadTeams().filter((item) => item.id !== id),200); renderAdvancedStudio(); showToast("Team deleted"); return true; }
