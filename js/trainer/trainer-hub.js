...
function exerciseRecord(label) {
  ...
  return LIBRARY.find((exercise) => exercise.name.toLowerCase() === name) || null;
}
...
function renderTrainerDirectory() {
  ...
  const names = trainerClientNames().filter((name) => name.toLowerCase().includes(query)); container.innerHTML = "";
  ...
  names.forEach((name) => {
    ...
    button.append(el("strong", "", name), el("span", "", (profile ? "@" + profileUsername(profile) + " · " : "") + goals), el("span", waiting || (lastReview && ["changed", "stopped"].includes(lastReview.data.pain)) ? "client-alert" : "", rowStatus));
...
function trainerAnalysisData(client) {
  ...
  const exercises = [...groups.entries()].map(([label, exerciseSets]) => {
    ...
    return { label, latest, trend, estimate: latestEstimate, exercise: LIBRARY.find((item) => item.name === label) || null };
  });
  ...
  const regions = regionDefinitions.map((region) => {
    const matches = recentSets.filter((entry) => {
      const exercise = LIBRARY.find((item) => item.name === entry.label); if (!exercise) return false;
  ...
  let priority = "Log completed sets and finish reviews to build a useful coaching baseline.";
  if (latestReview && ["changed", "stopped"].includes(latestReview.pain)) priority = "Safety first: keep " + (latestReview.injuryArea ? INJURY_LABELS[latestReview.injuryArea].toLowerCase() : "the painful area") + " filtered and do not repeat the aggravating movement unchanged. Refer out for sharp, worsening, or unexplained symptoms.";
  else if (readiness && readiness < 50) priority = "Readiness is currently low. Reduce volume, simplify exercise choices, and reassess during the warm-up.";
  else if (!workouts.length) priority = "Complete the first post-workout review so progress toward the client’s goal, effort, completion, and safety can be evaluated.";
  else if (["strength", "hypertrophy"].includes(primaryGoal) && averageTrend == null) priority = "Repeat key lifts across separate workouts to establish a strength trend relative to the client’s goal.";
  else if (completion < 70) priority = "Completion is below target. Shorten the next session or reduce accessory volume before adding load.";
  else if (effortFit < 65) priority = "Effort is missing the goal’s target range. Adjust load so the workout lands closer to RPE " + targetDifficulty + ".";
  else if (averageTrend != null && averageTrend > 0) priority = "Strength is trending upward. Progress one variable at a time while preserving completion and technique.";
  else priority = "The plan is matching the goal. Repeat successful loads and progress the exercises that remain controlled.";
  return { client, profile, entries, workouts, sets, strengthSets, recentWorkouts, recentSets, exercises, goals, primaryGoal, completion, consistency, effortFit, readiness, safety, strengthProgress, averageTrend, comparableExercises, evidenceLevel, evidenceDetail, regions, injuryAreas, priority };
}
...
function normalizedTrainerSummaryTab(tab) {
  if (["program","history"].includes(tab)) return "workouts";
  if (["strength","inbody"].includes(tab)) return "progress";
  if (["messages","checkins","coaching"].includes(tab)) return "communication";
  if (["assessments","safety","nutrition","notes","documents"].includes(tab)) return "details";
  return ["overview","workouts","progress","communication","details"].includes(tab) ? tab : "overview";
}
function setTrainerSummaryTab(tab) { trainerSummaryState.tab = normalizedTrainerSummaryTab(tab); renderTrainerAnalysis(selectedTrainerClient); }
function setTrainerSummaryView(view) { trainerSummaryState.view = view; renderTrainerAnalysis(selectedTrainerClient); }
function chooseSummaryCategory(category) { trainerSummaryState.category = trainerSummaryState.category === category ? "all" : category; trainerSummaryState.tab = "strength"; renderTrainerAnalysis(selectedTrainerClient); }
...
function summaryMetricsHtml(filtered) {
  ...
  const readiness = filtered.readiness[0] ? filtered.readiness[0].value : '—';
  return '<div class="summary-filtered-metrics"><div class="summary-filtered-metric"><b>' + filtered.workouts.length + '</b><span>Reviewed workouts</span></div><div class="summary-filtered-metric"><b>' + filtered.sets.length + '</b><span>Logged sets</span></div><div class="summary-filtered-metric"><b>' + avg + '</b><span>Average difficulty</span></div><div class="summary-filtered-metric"><b>' + escapeHtml(readiness) + '</b><span>Latest readiness</span></div></div>';
}
function sessionDetailHtml(session) {
  ...
  const assignment = assignmentForSession(session.key), stats = assignment ? assignmentProgressStats(assignment) : null, data = review && review.data || assignment && assignment.clientReview || null;
  const reviewHtml = data ? '<div class="workout-review-summary"><div class="simple-stat-grid"><div class="simple-stat"><b>' + (stats ? stats.logged + '/' + stats.planned : data.loggedSets || '—') + '</b><span>Completed / planned sets</span></div><div class="simple-stat"><b>' + (data.actualDuration || data.duration || '—') + '</b><span>Minutes</span></div><div class="simple-stat"><b>' + (data.difficulty || '—') + (data.difficulty ? '/10' : '') + '</b><span>Difficulty</span></div><div class="simple-stat"><b>' + (data.energy || '—') + (data.energy ? '/5' : '') + '</b><span>Energy after</span></div></div><div class="priority-card"><b>' + (data.completion === 'all' ? 'Workout completed as planned' : data.completion ? 'Completion: ' + escapeHtml(data.completion) : 'Workout feedback saved') + '</b><br>' + (data.pain && data.pain !== 'none' ? 'Pain or discomfort: ' + escapeHtml(data.injuryArea || data.pain) + (data.injuryDetails ? ' · ' + escapeHtml(data.injuryDetails) : '') : 'No pain reported.') + (data.notes ? '<br>Client note: ' + escapeHtml(data.notes) : '') + (data.questions ? '<br>Client question: ' + escapeHtml(data.questions) : '') + ((data.personalRecords || []).length ? '<br>Personal bests: ' + escapeHtml(data.personalRecords.join(', ')) : '') + '</div></div>' : '<div class="summary-alert">The exercise log is saved, but the client finish review is not available.</div>';
  const exceptions = session.entries.filter((entry) => ["skipped_set","skipped_exercise","substitution","pain","note"].includes(entry.type));
  const exceptionHtml = exceptions.length ? '<div class="workout-exceptions"><h5>Changes, skips &amp; notes</h5>' + exceptions.map((entry) => '<div class="analysis-history-item"><b>' + escapeHtml(entry.label || entry.type) + '</b><span>' + escapeHtml(entry.value || entry.note || entry.type.replace(/_/g,' ')) + '</span></div>').join('') + '</div>' : '';
  return reviewHtml + exceptionHtml + '<div class="session-set-list">' + (rows || '<div class="summary-alert">No working sets were logged for this workout.</div>') + '</div>';
}
...
function summaryStrengthContent(filtered) {
  ...
  return '<h4 class="analysis-section-title">Upper, lower & performance categories</h4>' + summaryCategoryCards(filtered) + '<div class="summary-subcategories">' + subcategories.map((name) => '<span class="summary-subcategory">' + escapeHtml(name) + '</span>').join("") + '</div><section class="analysis-panel"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><div><h4 class="analysis-section-title">Strength detail</h4><p style="font-size:9px;color:#737780;margin-top:4px">Estimated maxima compare each exercise with its own history; rehabilitation and technique sets can be excluded.</p></div><div class="summary-view-toggle"><button class="tiny-btn" onclick="setTrainerSummaryView(\'chart\')">Chart</button><button class="tiny-btn" onclick="setTrainerSummaryView(\'table\')">Table</button></div></div>' + (trainerSummaryState.view === 'table' ? '<table class="analysis-table"><thead><tr><th>Exercise</th><th>Pattern</th><th>Latest</th><th>Best est.</th><th>Trend</th><th>Avg RPE</th><th>Controls</th></tr></thead><tbody>' + (rows || '<tr><td colspan="7">No included strength sets match these filters.</td></tr>') + '</tbody></table>' : (chart || '<div class="empty-state">No included strength sets match these filters.</div>')) + '</section>' + excludedHtml;
}
function summaryOverviewContent(analysis, filtered) {
  const profile = analysis.profile, assignment = profile && assignmentForClient(profile.id), status = assignment && assignmentStatus(assignment), waiting = status === "completed", active = ["assigned","in_progress"].includes(status), messages = profile ? loadClientMessages(profile.id) : [], lastClientMessage = messages.find((item) => messageSenderRole(item) === "client"), latestReview = filtered.workouts[0] && filtered.workouts[0].data;
  const actionTitle = waiting ? "Workout ready for review" : active ? assignmentStatusLabel(assignment) : "No active workout";
  const actionCopy = waiting ? "Review the client’s completion, effort, pain report, notes, and logged sets before building the next session." : active ? "The current workout remains the client’s only active assignment. Monitor progress or open its history when needed." : "Build one clear workout when this client is ready for their next session.";
  const actionButton = waiting ? '<button class="small-btn primary" onclick="openCoachAdjustment(\'' + escapeHtml(profile.id) + '\')">Review workout</button>' : active ? '<button class="small-btn" onclick="setTrainerSummaryTab(\'workouts\')">Open workout</button>' : '<button class="small-btn primary" onclick="openSelectedClientSession()">Build next workout</button>';
  return '<section class="client-overview-action ' + (waiting ? 'needs-action' : '') + '"><div><span>Next coaching action</span><h4>' + escapeHtml(actionTitle) + '</h4><p>' + escapeHtml(actionCopy) + '</p></div><div class="tool-actions">' + actionButton + '</div></section>'
    + summaryMetricsHtml(filtered)
    + '<div class="analysis-grid overview-priority-grid"><section class="analysis-panel"><h4 class="analysis-section-title">Latest workout</h4>' + (latestReview ? '<div class="simple-stat-grid"><div class="simple-stat"><b>' + (latestReview.loggedSets || 0) + '</b><span>Sets logged</span></div><div class="simple-stat"><b>' + (latestReview.difficulty || '—') + (latestReview.difficulty ? '/10' : '') + '</b><span>Difficulty</span></div><div class="simple-stat"><b>' + (latestReview.energy || '—') + (latestReview.energy ? '/5' : '') + '</b><span>Energy</span></div><div class="simple-stat"><b>' + (latestReview.pain === 'none' ? 'None' : escapeHtml(latestReview.injuryArea || latestReview.pain || '—')) + '</b><span>Pain</span></div></div><div class="tool-actions"><button class="small-btn" onclick="setTrainerSummaryTab(\'workouts\')">Open full history</button></div>' : '<div class="empty-state">No completed workout review yet.</div>') + '</section><aside class="analysis-panel"><h4 class="analysis-section-title">Coach priority</h4><div class="priority-card">' + escapeHtml(analysis.priority) + '</div><div class="analysis-history-item" style="margin-top:12px"><b>' + messages.filter((item) => messageSenderRole(item) === 'client').length + ' client message' + (messages.filter((item) => messageSenderRole(item) === 'client').length === 1 ? '' : 's') + '</b><span>' + escapeHtml(lastClientMessage ? 'Latest: ' + lastClientMessage.body : 'No client message yet.') + '</span></div><div class="tool-actions"><button class="small-btn" onclick="setTrainerSummaryTab(\'communication\')">Open communication</button></div></aside></div>'
    + '<section class="analysis-panel" style="margin-top:14px"><h4 class="analysis-section-title">Recent workout history</h4>' + sessionTimelineHtml(filtered.sessions,3) + '</section>';
}
...
function trainerIntakeSummaryHtml(profile) {
  if (!profile) return "";
  const intake = profile.intake || {}, status = intakeCompletion(profile,intake), health = intake.health || {}, triggered = INTAKE_SAFETY_FIELDS.filter((key) => health[key] === "yes");
  const checkLabels = {identity:"Identity",goals:"Goals",schedule:"Schedule",equipment:"Equipment",health:"Health readiness",lifestyle:"Lifestyle",experience:"Coaching fit",emergency:"Emergency contact",consent:"Consent"};
  const decision = intake.trainerDecision === "ready" ? "Ready to program" : intake.trainerDecision === "modified" ? "Ready with modifications" : intake.trainerDecision === "hold" ? "Hold and refer / clarify" : "Not documented";
  return trainerGoalContractSummaryHtml(profile) + '<section class="analysis-panel" style="margin-top:14px"><div class="analysis-panel-head"><div><h4 class="analysis-section-title">Onboarding & health-readiness review</h4><p style="font-size:9px;color:var(--text-faint);margin-top:4px">' + status.complete + ' of ' + status.total + ' sections ready · ' + status.percent + '% complete</p></div><button class="small-btn primary" onclick="openClientIntake(\'' + escapeHtml(profile.id) + '\',\'trainer\')">Open full intake</button></div>'
    + '<div class="simple-stat-grid"><div class="simple-stat"><b>' + status.percent + '%</b><span>Onboarding</span></div><div class="simple-stat"><b>' + (!status.checks.find((check) => check.key === "health").ready ? 'Incomplete' : status.safetyRequired ? status.safetyReviewed ? 'Reviewed' : 'Required' : 'Clear') + '</b><span>Safety review</span></div><div class="simple-stat"><b>' + escapeHtml(decision) + '</b><span>Trainer decision</span></div><div class="simple-stat"><b>' + escapeHtml(intake.coachingStyle || '—') + '</b><span>Coaching style</span></div></div>'
    + '<div class="intake-checks" style="margin-top:12px">' + status.checks.map((check) => '<span class="' + (check.ready ? 'ready' : '') + '">' + (check.ready ? '✓ ' : '○ ') + escapeHtml(checkLabels[check.key] || check.key) + '</span>').join("") + '</div>'
    + '<div class="analysis-grid" style="margin-top:12px"><div class="analysis-history"><div class="analysis-history-item"><b>Training context</b><span>' + escapeHtml([intake.location,intake.occupation,intake.barriers].filter(Boolean).join(' · ') || 'Not entered') + '</span></div><div class="analysis-history-item"><b>Recovery context</b><span>' + escapeHtml([intake.sleep,intake.stress,intake.activity].filter(Boolean).join(' · ') || 'Not entered') + '</span></div></div><div class="analysis-history"><div class="analysis-history-item"><b>Health flags</b><span>' + escapeHtml(triggered.length ? triggered.map((key) => key.replace(/([A-Z])/g,' $1').toLowerCase()).join(' · ') : 'No “yes” answers entered') + '</span></div><div class="analysis-history-item"><b>Current pain / restriction</b><span>' + escapeHtml(intake.currentPain || 'None entered') + '</span></div><div class="analysis-history-item"><b>Trainer note</b><span>' + escapeHtml(intake.trainerNote || 'No decision note yet') + '</span></div></div></div>'
    + (status.approvalBlocked ? '<div class="summary-alert"><b>Programming approval is paused.</b><br>' + (status.clientComplete ? 'Review the completed intake and document the trainer baseline decision.' : 'Finish the missing client onboarding sections, then document the trainer baseline decision.') + ' This is a coaching safety screen, not a medical diagnosis.</div>' : '<div class="summary-alert safe"><b>Onboarding and trainer review complete.</b><br>Continue to check current symptoms and readiness before each workout.</div>') + '</section>';
}
function trainerAssignmentLoopHtml(profile) {
  ...
  else actions = '<button class="small-btn" onclick="setTrainerSummaryTab(\'workouts\')">View current workout</button>';
...
function clientCheckInTypeLabel(item) {
  return item.reviewType === "recovery_24_48" ? "24–48h recovery pulse" : item.reviewType === "starter_week_1" ? "Week 1 starter review" : "Weekly check-in";
}
...
function trainerNotesTab(analysis) { const notes = analysis.entries.filter((entry) => entry.type === 'note' || summaryMetaFor(entry).note); return '<section class="analysis-panel"><h4 class="analysis-section-title">Coach & workout notes</h4><div class="analysis-history">' + (notes.map((entry) => '<div class="analysis-history-item"><b>' + escapeHtml(entry.label || 'Note') + ' · ' + new Date(entry.date).toLocaleDateString() + '</b><span>' + escapeHtml(entry.note || summaryMetaFor(entry).note || entry.value || '') + '</span></div>').join('') || '<div class="empty-state">No saved notes.</div>') + '</div></section>'; }
...
function trainerCommunicationTab(profile) {
  return '<div class="trainer-communication-stack">' + trainerMessagesTab(profile) + trainerCheckInsTab(profile) + '</div>';
}
function trainerClientDetailsTab(analysis,profile,filtered) {
  if (!profile) return '<section class="analysis-panel"><div class="empty-state">This history record is not connected to a saved client profile.</div></section>';
  const goalLabels = (profile.goals || []).map((goal) => GOALS[goal] ? GOALS[goal].label : goal).join(' + ') || 'General fitness';
  const injuries = (profile.injuries || []).map((item) => INJURY_LABELS[item] || item).join(', ') || 'None recorded';
  const equipment = (profile.zones || []).map((item) => ZONE_LABELS[item] || item).join(', ') || 'Full gym / not restricted';
  const days = inferredTrainingDays(profile,profile.availableDays || 3).map((day) => ['','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][day]).filter(Boolean).join(', ') || 'Not set';
  return '<section class="analysis-panel client-details-card"><div class="analysis-panel-head"><div><h4 class="analysis-section-title">Client details</h4><p>Profile facts and coaching constraints stay together here.</p></div><button class="small-btn primary" onclick="openProfileEditor(\'' + escapeHtml(profile.id) + '\')">Edit profile</button></div><div class="client-detail-grid"><div><span>Primary goal</span><b>' + escapeHtml(goalLabels) + '</b></div><div><span>Experience</span><b>' + escapeHtml(EXP_LABEL(profile.experience)) + '</b></div><div><span>Typical session</span><b>' + escapeHtml((profile.minutes || 60) + ' minutes') + '</b></div><div><span>Usual days</span><b>' + escapeHtml(days) + '</b></div><div><span>Limitations</span><b>' + escapeHtml(injuries) + '</b></div><div><span>Equipment</span><b>' + escapeHtml(equipment) + '</b></div><div><span>Assigned trainer</span><b>' + escapeHtml(profile.assignedTrainerName || 'Coaching team') + '</b></div><div><span>Contact</span><b>' + escapeHtml(profile.email || 'Not recorded') + '</b></div></div></section>'
    + '<section class="analysis-panel" style="margin-top:14px"><h4 class="analysis-section-title">Safety & readiness</h4>' + summarySafetyContent(filtered) + '</section>'
    + '<div class="analysis-grid" style="margin-top:14px">' + trainerNotesTab(analysis) + trainerDocumentsTab(analysis) + '</div>'
    + '<section class="analysis-panel" style="margin-top:14px"><h4 class="analysis-section-title">Measurements & optional support</h4><div class="tool-actions"><button class="small-btn" onclick="openInBodyModal()">+ InBody scan</button><button class="small-btn" onclick="openBodyGoalModal()">Body goals</button></div></section>'
    + '<details class="client-danger-zone"><summary>Archive or delete client data</summary><p>These controls are kept away from everyday coaching actions. Export a backup before permanently deleting client records.</p><div class="tool-actions"><button class="small-btn danger" onclick="deleteClientProfile(\'' + escapeHtml(profile.id) + '\')">Delete profile only</button><button class="small-btn danger" onclick="openCompleteDeleteClient(decodeURIComponent(\'' + encodeURIComponent(analysis.client) + '\'))">Delete all client data</button></div></details>';
}
function renderTrainerAnalysis(client) {
  ...
  trainerSummaryState.tab = normalizedTrainerSummaryTab(trainerSummaryState.tab);
  const tabs = [["overview","Overview"],["workouts","Workouts"],["progress","Progress"],["communication","Communication"],["details","Client details"]];
  let content = trainerAssignmentLoopHtml(profile) + summaryOverviewContent(analysis,filtered);
  if (trainerSummaryState.tab === "workouts") content = trainerAssignmentLoopHtml(profile) + trainerFilterPanel(analysis,filtered) + summaryHistoryContent(filtered);
  else if (trainerSummaryState.tab === "progress") content = trainerProgressReceiptsHtml(profile) + summaryStrengthContent(filtered) + '<details class="formal-review-box"><summary>Body composition & InBody progress</summary>' + renderInBodyContent(analysis) + '</details>';
  else if (trainerSummaryState.tab === "communication") content = trainerCommunicationTab(profile);
  else if (trainerSummaryState.tab === "details") content = trainerClientDetailsTab(analysis,profile,filtered);
  const currentAssignment = profile && assignmentForClient(profile.id), statusLabel = currentAssignment ? assignmentStatusLabel(currentAssignment) : "No active workout";
  out.innerHTML = '<article class="analysis-sheet simplified-client-sheet"><header class="analysis-top"><div class="analysis-client"><h3>' + escapeHtml(client) + '</h3><span>' + (profile ? '@' + escapeHtml(profileUsername(profile)) + ' · Age ' + profile.age + ' · ' + EXP_LABEL(profile.experience) : 'History-only client') + '</span></div><div class="analysis-header-status"><span class="assignment-pill ' + escapeHtml(currentAssignment ? assignmentStatus(currentAssignment) : '') + '">' + escapeHtml(statusLabel) + '</span><div class="analysis-goals">Goal · ' + escapeHtml(goalLabels) + '</div></div></header><nav class="summary-tabs simplified-tabs">' + tabs.map(([key,label]) => '<button class="summary-tab' + (trainerSummaryState.tab === key ? ' on' : '') + '" onclick="setTrainerSummaryTab(\'' + key + '\')">' + label + '</button>').join("") + '</nav><div class="analysis-body">' + content + '<p class="analysis-note">Workout summaries use this client’s saved sets, finish reviews, readiness checks, and optional measurements. They are coaching records—not medical diagnoses or population rankings.</p></div></article>';
...
function fullInBodyAnalysisHtml(scan, isLatest) {
  ...
  return '<section class="analysis-panel" style="margin-bottom:14px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start"><div><h4 class="analysis-section-title">Complete InBody analysis</h4><p style="font-size:9px;color:#737780;margin-top:4px">Every saved result from this scan, grouped for trainer review.</p></div><span class="summary-subcategory">' + (isLatest ? 'Latest scan' : 'Selected history scan') + '</span></div><div class="inbody-subject-strip">' + subjectItems.map(([label,value]) => '<span>' + label + ': <b>' + escapeHtml(value) + '</b></span>').join("") + '</div><div class="inbody-full-grid">'
...
function renderInBodyContent(analysis) {
  ...
  const trendVisual = goalData ? '<section class="analysis-panel" style="margin-bottom:14px"><h4 class="analysis-section-title">Weight & body-fat trend</h4><p style="font-size:9px;color:#737780;margin-top:4px">Separate panels keep the two goals readable without combining them into one score.</p><canvas id="bodyGoalTrendCanvas" class="body-trend-canvas" role="img" aria-label="Body weight and body fat trend toward saved goals"></canvas></section>' : '';
