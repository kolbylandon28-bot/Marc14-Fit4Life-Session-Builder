...
function buildReadinessSession() {
  ...
  const spec = {
    ...
    minutes: Number(byId("readyMinutes").value), muscles: recoveryOnly ? [] : READINESS_FOCUS[byId("readyFocus").value], injuries: [...(profile && profile.injuries || [])], zones: [...(profile && profile.zones || [])], trainingPhase:profile && profile.trainingPhase || "general", sport:profile && profile.sport || "", sportSchedule:profile && profile.sportSchedule || "", competitionDate:profile && profile.competitionDate || "", exercisePreferences:{ ...(profile && profile.exercisePreferences || {}) }, phaseCompoundAnchors:{ ...(profile && profile.phaseCompoundAnchors || {}) },
  };
...
function paintClientIntakeStatus() {
  ...
  const trainerMode = byId("clientIntakeDialog").classList.contains("trainer-mode"), intake = intakeFromFields(profile.intake || {},trainerMode), status = intakeCompletion(profile,intake), safety = byId("clientIntakeSafetyMessage");
...
function goToNextIncompleteIntakeSection() {
  ...
  const trainerMode = byId("clientIntakeDialog").classList.contains("trainer-mode"), status = intakeCompletion(profile,intakeFromFields(profile.intake || {},trainerMode));
...
function openClientIntake(profileId,mode) {
  ...
  const trainerMode = mode === "trainer" || portalRole === "trainer";
  byId("clientIntakeProfileId").value = profile.id;
  byId("clientIntakeTitle").textContent = (trainerMode ? "Review " : "") + profile.name + " · onboarding";
  byId("clientIntakeCopy").textContent = trainerMode ? "Review what the client submitted, document any safety decision, and finish missing coaching context." : "Complete one short section at a time. Save now and return whenever you need.";
...
function openClientOnboarding() { const profile = activeClientProfile(); if (profile) openClientIntake(profile.id,"client"); }
...
function saveClientIntake() {
  ...
  const trainerMode = byId("clientIntakeDialog").classList.contains("trainer-mode"), previous = profiles[index].intake || {}, intake = intakeFromFields(previous,trainerMode);
  ...
  const approvedProfileUpdates = trainerMode ? {
    email:profiles[index].email || intake.identity && intake.identity.email || "",
    goals:intake.goals && intake.goals.length ? [...intake.goals] : profiles[index].goals,
    availableDays:intake.schedule && intake.schedule.daysPerWeek || profiles[index].availableDays,
    minutes:intake.schedule && intake.schedule.sessionMinutes || profiles[index].minutes,
    trainingDays:intake.schedule && intake.schedule.trainingDays && intake.schedule.trainingDays.length ? [...intake.schedule.trainingDays] : profiles[index].trainingDays,
    zones:intake.equipmentZones && intake.equipmentZones.length ? [...intake.equipmentZones] : profiles[index].zones,
  } : {};
  profiles[index] = {...profiles[index],...approvedProfileUpdates,intake,updatedAt:new Date().toISOString()};
  ...
  showToast(status.programmingBlocked ? "Onboarding saved · trainer safety decision required before difficult programming" : status.status === "complete" ? "Onboarding and trainer review complete" : status.status === "trainer_review" ? "Client onboarding submitted · trainer review is next" : "Onboarding saved · " + status.percent + "% complete");
  return intake;
}
...
function profileRecordFromTarget(target, name, username) {
  ...
  return {
    ...
    muscles: [...(target.muscles || [])], injuries: [...(target.injuries || [])], zones: [...(target.zones || [])],
    ...
    exercisePreferences:{ ...(target.exercisePreferences || {}) }, phaseCompoundAnchors:{ ...(target.phaseCompoundAnchors || {}) },
  };
}
...
function updateClientProfile(profileId, updates) {
  ...
  const next = { ...previous, ...updates, name, username, goals: goals.length ? goals : ["general"], trainingStyle:updates.trainingStyle || previous.trainingStyle || "auto", cardioMode:cardioModes[0], cardioModes, experience: Number(updates.experience) || 1, age: Number(updates.age) || 30, minutes: Number(updates.minutes) || 60, muscles: [...(updates.muscles || previous.muscles || [])], injuries: [...(updates.injuries || previous.injuries || [])], zones: [...(updates.zones || previous.zones || [])], trainingPhase:updates.trainingPhase || previous.trainingPhase || "general", phaseStartedAt:phaseChanged ? new Date().toISOString() : previous.phaseStartedAt || previous.updatedAt || new Date().toISOString(), availableDays, trainingDays:trainingDays.length ? trainingDays : inferredTrainingDays(previous,availableDays), sport:String(updates.sport != null ? updates.sport : previous.sport || "").trim(), sportSchedule:String(updates.sportSchedule != null ? updates.sportSchedule : previous.sportSchedule || "").trim(), competitionDate:updates.competitionDate != null ? updates.competitionDate : previous.competitionDate || "", exercisePreferences:{ ...(updates.exercisePreferences || previous.exercisePreferences || {}) }, phaseCompoundAnchors:phaseChanged ? {} : { ...(updates.phaseCompoundAnchors || previous.phaseCompoundAnchors || {}) }, updatedAt: new Date().toISOString() };
...
function toggleProfileEditorChoice(key, value) {
  if (!profileEditorDraft[key]) return; const index = profileEditorDraft[key].indexOf(value); if (index >= 0) profileEditorDraft[key].splice(index,1); else profileEditorDraft[key].push(value); renderProfileEditorChoices();
}
...
function openCreateProfileEditor() {
  if (!requireTrainerMutation("create client profiles")) return null;
  profileEditorTarget = null; profileEditorDraft = { muscles:[],injuries:[],zones:[],preferences:{},cardioModes:["any"],trainingDays:[1,3,5] };
...
function saveProfileEditor() {
  ...
  const creating = !profileId, profile = creating ? createClientProfile(updates) : updateClientProfile(profileId,updates);
  if (!profile) return null;
  if (profileEditorTarget) loadProfileIntoTarget(profile,profileEditorTarget);
  byId("profileEditorModal").classList.remove("open"); profileEditorTarget = null; refreshProfileSelects(); renderForms(); renderTrainerHub(profile.name); showToast(creating ? "Created " + profile.name + " without assigning a workout or program" : "Updated " + profile.name + " and kept all connected history"); return profile;
}
