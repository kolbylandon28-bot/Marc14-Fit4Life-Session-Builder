...
function setTrainerSummaryTab(tab) { trainerSummaryState.tab = tab; renderTrainerAnalysis(selectedTrainerClient); }
...
function replyToClientCheckIn(checkInId) {
  ...
  const suggested = painHold || item.painTrend === "worse"
  ...
  const identity = currentAccountIdentity(), body = window.prompt("Reply to " + profile.name + " as " + identity.displayName + ":",suggested); if (!body || !body.trim()) return null;
  const messages = loadLocalArray(CLIENT_MESSAGES_KEY);
  messages.unshift({id:"message-" + Date.now(),profileId:profile.id,client:profile.name,from:"coach",senderRole:identity.role === "owner" ? "owner" : "trainer",senderUserId:identity.id || "",senderName:identity.displayName,recipientRole:"client",recipientUserId:"",recipientName:profile.name,body:body.trim(),createdAt:new Date().toISOString(),sourceCheckInId:item.id});
  if (!writeLocalArray(CLIENT_MESSAGES_KEY,messages,1000)) return null;
  const reviewed = markClientCheckInReviewed(item.id,body.trim()); if (!reviewed) return null;
  showToast("Reply sent and recovery pulse marked reviewed"); return reviewed;
}
function trainerCheckInsTab(profile) {
  const items = profile ? checkInsForProfile(profile.id) : [];
  return '<section class="analysis-panel"><h4 class="analysis-section-title">Check-ins &amp; recovery</h4><p style="font-size:9px;color:var(--text-faint);margin:4px 0 12px">Recovery pulses are short post-workout signals. Weekly check-ins remain the broader coaching review.</p><div class="analysis-history">' + (items.map((item) => '<div class="analysis-history-item"><div><b>' + new Date(item.createdAt || item.date).toLocaleDateString() + ' · ' + escapeHtml(clientCheckInTypeLabel(item)) + (item.reviewedAt ? ' · reviewed' : ' · needs review') + '</b><span>' + escapeHtml(clientCheckInSummary(item)) + '</span>' + (item.reviewedAt ? '<span>Reviewed by ' + escapeHtml(item.reviewedBy || "trainer") + (item.trainerReply ? ' · reply sent' : '') + '</span>' : '') + '</div>' + (!item.reviewedAt ? '<div class="tool-actions"><button class="small-btn primary" onclick="replyToClientCheckIn(\'' + escapeHtml(item.id) + '\')">Reply &amp; review</button><button class="small-btn" onclick="markClientCheckInReviewed(\'' + escapeHtml(item.id) + '\');showToast(\'Check-in marked reviewed\')">Mark reviewed</button></div>' : '') + '</div>').join('') || '<div class="empty-state">No check-ins or recovery pulses submitted.</div>') + '</div></section>';
}
function trainerMessagesTab(profile) { const items = profile ? loadClientMessages(profile.id) : []; return '<section class="analysis-panel"><h4 class="analysis-section-title">Direct messages · ' + escapeHtml(profile && profile.assignedTrainerName || 'Coaching team') + '</h4>' + (profile ? messageThreadHtml(items,profile,'trainer') : '<div class="empty-state">Choose a client.</div>') + (profile ? '<div class="tool-actions"><button class="small-btn primary" onclick="replyToClientMessage(\'' + escapeHtml(profile.id) + '\')">Message ' + escapeHtml(profile.name) + '</button></div>' : '') + '</section>'; }
...
function renderTrainerAnalysis(client) {
  ...
  const tabs = [["overview","Overview"],["coaching","Coaching"],["program","Program"],["workouts","Workouts"],["progress","Progress"],["checkins","Check-ins"],["messages","Messages"],["assessments","Assessments"],["nutrition","Nutrition"],["notes","Notes"],["documents","Documents"]];
  let content = summaryOverviewContent(analysis, filtered);
  if (trainerSummaryState.tab === "coaching") content = trainerCoachingTab(profile);
  else if (trainerSummaryState.tab === "program") content = trainerProgramTab(profile);
  else if (["workouts","history"].includes(trainerSummaryState.tab)) content = summaryHistoryContent(filtered);
  else if (["progress","strength"].includes(trainerSummaryState.tab)) content = (trainerSummaryState.tab === "progress" ? trainerProgressReceiptsHtml(profile) : "") + summaryStrengthContent(filtered) + (trainerSummaryState.tab === "progress" ? '<details class="formal-review-box"><summary>Body composition & InBody progress</summary>' + renderInBodyContent(analysis) + '</details>' : '');
  ...
  else if (["assessments","safety"].includes(trainerSummaryState.tab)) content = summarySafetyContent(filtered) + trainerIntakeSummaryHtml(profile);
  else if (trainerSummaryState.tab === "inbody") content = renderInBodyContent(analysis);
  else if (trainerSummaryState.tab === "nutrition") content = trainerNutritionTab(profile);
  else if (trainerSummaryState.tab === "notes") content = trainerNotesTab(analysis);
  else if (trainerSummaryState.tab === "documents") content = trainerDocumentsTab(analysis);
  if (["overview","program","assessments"].includes(trainerSummaryState.tab)) content = baselineTrainerCardHtml(profile) + content;
  const filterTabs = ["overview","workouts","history","progress","strength","assessments","safety"];
  out.innerHTML = '<article class="analysis-sheet"><header class="analysis-top"><div class="analysis-client"><h3>' + escapeHtml(client) + '</h3><span>' + (profile ? '@' + escapeHtml(profileUsername(profile)) + ' · Age ' + profile.age + ' · ' + EXP_LABEL(profile.experience) : 'History-only client') + '</span></div><div class="analysis-goals">Goal · ' + escapeHtml(goalLabels) + '</div></header><nav class="summary-tabs">' + tabs.map(([key,label]) => '<button class="summary-tab' + (trainerSummaryState.tab === key ? ' on' : '') + '" onclick="setTrainerSummaryTab(\'' + key + '\')">' + label + '</button>').join("") + '</nav><div class="analysis-body">' + trainerAssignmentLoopHtml(profile) + (filterTabs.includes(trainerSummaryState.tab) ? trainerFilterPanel(analysis, filtered) : '') + content
    + '<div class="analysis-actions"><button class="small-btn primary" onclick="openSelectedClientSession()">Build next workout</button><button class="small-btn" onclick="openSelectedClientProgram()">Build program</button>' + (profile ? '<button class="small-btn" onclick="openProfileEditor(\'' + escapeHtml(profile.id) + '\')">Edit profile</button><button class="small-btn danger" onclick="deleteClientProfile(\'' + escapeHtml(profile.id) + '\')">Delete profile only</button>' : '') + '<button class="small-btn danger" onclick="openCompleteDeleteClient(decodeURIComponent(\'' + encodeURIComponent(client) + '\'))">Delete all client data</button><button class="small-btn" onclick="openInBodyModal()">+ InBody scan</button><button class="small-btn" onclick="openBodyGoalModal()">Body goals</button>' + (state.session ? '<button class="small-btn" onclick="openWorkoutReview()">Review current workout</button>' : '') + '</div><p class="analysis-note">Coaching summaries are derived from this client’s logged FIT4LIFE sets, readiness checks, workout reviews, and saved InBody values. They are not population percentiles, medical diagnoses, or laboratory measurements.</p></div></article>';
  if (["progress","inbody"].includes(trainerSummaryState.tab) && typeof setTimeout === "function") setTimeout(() => { drawHumanScanRadarChart(); drawBodyGoalTrendChart(); }, 0);
  return analysis;
}
